...
internal static class TransactionHttpLoadRunner
{
    public static async Task RunAsync(string[] args)
    {
        ...
        Console.WriteLine($"Target RPS: {options.Rate} | Duration: {options.DurationSeconds}s | Persistence mean latency gate: < {TransactionSlo.MaxMeanLatencyMs}ms");
        Console.WriteLine(
            $"OK: {result.OkCount} | Fail: {result.FailCount} | RPS: {result.OkRps:F1} | Mean latency: {result.MeanLatencyMs:F1}ms");

        if (result.FailCount > 0)
        {
            throw new InvalidOperationException($"HTTP load test had {result.FailCount} failed requests.");
        }

        if (result.MeanLatencyMs > TransactionSlo.MaxMeanLatencyMs)
        {
            throw new InvalidOperationException(
                $"Mean latency {result.MeanLatencyMs:F1}ms exceeded persistence target of {TransactionSlo.MaxMeanLatencyMs}ms.");
        }
    }
}
