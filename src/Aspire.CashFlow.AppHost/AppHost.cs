...
var cognitoRegion = builder.AddParameter(
    "cognito-region",
    RequireSetting(configuration, "CASHFLOW_COGNITO_REGION", "Parameters:cognito-region", "Cognito:Region")
);
...
var cognitoServiceUrl = builder.AddParameter(
    "cognito-service-url",
    RequireSetting(configuration, "CASHFLOW_COGNITO_SERVICE_URL", "Parameters:cognito-service-url", "Cognito:ServiceUrl")
);
...
var cognitoClientId = builder.AddParameter(
    "cognito-client-id",
    RequireSetting(configuration, "CASHFLOW_COGNITO_CLIENT_ID", "Parameters:cognito-client-id", "Cognito:ClientId")
);
...
var secretsPrefix = builder.AddParameter(
    "secrets-prefix",
    RequireSetting(configuration, "SECRETS_PREFIX", "Parameters:secrets-prefix", "SecretsManager:Prefix")
);

var secretsServiceUrl = builder.AddParameter(
    "secrets-service-url",
    RequireSetting(configuration, "SECRETS_MANAGER_SERVICE_URL", "Parameters:secrets-service-url", "SecretsManager:ServiceUrl")
);

var kmsDefaultKeyId = builder.AddParameter(
    "kms-default-key-id",
    RequireSetting(configuration, "KMS_DEFAULT_KEY_ID", "Parameters:kms-default-key-id", "Kms:DefaultKeyId")
);
...
var demoAccountEmail = RequireSetting(
    configuration,
    "COGNITO_USERNAME",
    "DemoAccount:Email"
);

var demoAccountPassword = RequireSetting(
    configuration,
    "COGNITO_PASSWORD",
