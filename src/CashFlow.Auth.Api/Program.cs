...
using InfrastructureCognitoOptions = CashFlow.Auth.Infrastructure.Configuration.CognitoOptions;
...
var cognitoOptions = app.Services.GetRequiredService<IOptions<InfrastructureCognitoOptions>>().Value;
    ...
    CancellationToken cancellationToken) =>
{
    ...
    {
        RecordLoginEvent(securityAuditService, authMetrics, request.Email, "failed", "missing-credentials");
    ...
    {
        RecordLoginEvent(securityAuditService, authMetrics, request.Email, "mfa-required", "challenge-issued");
        return Results.Ok(result);
    }

    if (result.Succeeded)
    {
        RecordLoginEvent(securityAuditService, authMetrics, request.Email, "succeeded", "token-issued");
        return Results.Ok(result);
    }

    RecordLoginEvent(securityAuditService, authMetrics, request.Email, "failed", result.ErrorMessage ?? "invalid-credentials");
    return Results.Problem(
    ...
    CancellationToken cancellationToken) =>
{
    ...
    {
        RecordRefreshEvent(securityAuditService, authMetrics, "failed", "missing-refresh-token");
        return Results.Problem(
            title: "Invalid request",
    ...
    var result = await authenticationService.RefreshSessionAsync(request.RefreshToken, cancellationToken);
    if (result.Succeeded)
    {
    ...
    RecordRefreshEvent(securityAuditService, authMetrics, "failed", result.ErrorMessage ?? "invalid-refresh-token");
    return Results.Problem(
...
app.MapGet("/api/auth/session", [Authorize] (
    ClaimsPrincipal user,
    IOptions<InfrastructureCognitoOptions> options,
    SecurityAuditService securityAuditService,
    AuthMetrics authMetrics) =>
{
    ...
    string reason)
{
    securityAuditService.Record(new SecurityEventRecord
    {
        EventType = outcome == "succeeded" ? AuthMetrics.LoginSucceeded : AuthMetrics.LoginFailed,
        Outcome = outcome,
    ...
    string reason)
{
    securityAuditService.Record(new SecurityEventRecord
    {
        EventType = outcome == "succeeded" ? AuthMetrics.TokenRefreshSucceeded : AuthMetrics.TokenRefreshFailed,
        Outcome = outcome,
...
static SessionState MapSession(ClaimsPrincipal user, InfrastructureCognitoOptions cognitoOptions)
{
    ...
    return new SessionState
    {
        ...
        AuthenticationSource = user.FindFirstValue("auth_source")
            ?? (cognitoOptions.IsConfigured ? "Cognito" : "InMemoryFallback"),
        MfaRequired = string.Equals(user.FindFirstValue("mfa_required"), bool.TrueString, StringComparison.OrdinalIgnoreCase),
