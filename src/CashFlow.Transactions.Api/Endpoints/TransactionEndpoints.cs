...
public static class TransactionEndpoints
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = await transactionService.CreateAsync(request, userId, idempotencyKey, cancellationToken);
        if (result.Succeeded)
        {
            return Results.Ok(result);
        }

        if (result.ErrorMessage?.Contains("could not be recorded", StringComparison.OrdinalIgnoreCase) == true)
        {
            return Results.Problem(
