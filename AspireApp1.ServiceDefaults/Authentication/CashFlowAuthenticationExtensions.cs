...
{
    private static readonly ConcurrentDictionary<string, IReadOnlyCollection<SecurityKey>> LocalStackSigningKeys = new(StringComparer.Ordinal);
    ...
    private static void ConfigureCognitoJwtBearer(JwtBearerOptions options, CognitoOptions cognitoOptions)
    {
        ...
        if (cognitoOptions.UseLocalStack)
        {
            ...
            options.TokenValidationParameters = new TokenValidationParameters
            {
                ...
                IssuerSigningKeyResolver = (_, _, keyId, _) => ResolveLocalStackSigningKeys(cognitoOptions, keyId)
            };
        }
        else
        {
    ...
    private static IEnumerable<SecurityKey> ResolveLocalStackSigningKeys(CognitoOptions cognitoOptions, string? keyId)
    {
        var keys = LocalStackSigningKeys.GetOrAdd(cognitoOptions.Authority, _ => LoadSigningKeys(cognitoOptions.JwksUri));
    ...
    private static IReadOnlyCollection<SecurityKey> LoadSigningKeys(Uri jwksUri)
    {
