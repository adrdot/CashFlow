...
public static class CashFlowSecurityExtensions
{
    ...
    {
        var securityOptions = configuration.GetSection(CashFlowSecurityOptions.SectionName).Get<CashFlowSecurityOptions>()
            ?? new CashFlowSecurityOptions();

        services.Configure<CashFlowSecurityOptions>(configuration.GetSection(CashFlowSecurityOptions.SectionName));
                ...
                options.AddPolicy(AuthLoginRateLimitPolicy, context =>
                    RateLimitPartition.GetFixedWindowLimiter(
                        ...
                        factory: _ => new FixedWindowRateLimiterOptions
                        {
                            ...
                            Window = TimeSpan.FromSeconds(securityOptions.AuthLoginWindowSeconds),
                            QueueLimit = 0
                        }));
            });
        }
    ...
    public static WebApplication UseCashFlowSecurity(this WebApplication app)
    {
        ...
        var rateLimitingEnabled = app.Configuration
            ...
            .Get<CashFlowSecurityOptions>()?.RateLimitingEnabled ?? true;
